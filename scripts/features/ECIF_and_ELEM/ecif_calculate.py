...
# https://github.com/DIFACQUIM/ECIF/blob/master/01_ECIF_Calculation.ipynb
# ========================================================================
...
def LoadSDFasDF(SDF):
	# This function takes an SDF for a ligand as input and
	#	 returns it as a pandas DataFrame with its atom types labeled according to ECIF
	...
	if len(set(df["ECIF_ATOM_TYPE"]) - set(ECIF_LigandAtoms)) > 0:
		...
		#print(set(df["ECIF_ATOM_TYPE"]) - set(ECIF_LigandAtoms))
		#for x in set(ECIF_LigandAtoms):
		#	if x not in set(df["ECIF_ATOM_TYPE"]):
		#		print(x)
	return(df)


def LoadPDBasDF(PDB):
	# This function takes a PDB for a protein as input and
	#   returns it as a pandas DataFrame with its atom types labeled according to ECIF
...
parser = argparse.ArgumentParser(description='calculate the ECIF')
...
parser.add_argument('-l', '--ligand', dest='ligand', required=True, 
					help='Input ligand file (.sdf)')
parser.add_argument('--prot_name', dest='prot_name', type=str,
					help='The name of the protein.')
parser.add_argument('--lig_name', dest='lig_name', type=str,
					help='The name of the ligand.')
parser.add_argument('--pdb_atom_keys_file', dest='pdb_atom_keys_file', default='./PDB_Atom_Keys.csv',
					help='The file recording the atom information of the protein. (default: "./PDB_Atom_Keys.csv")')								
parser.add_argument('-d', '--desc_type', dest='desc_type', default='all', choices=['all','ecif','elem','lig'],
					help='Which descritpors to generate.')

args = parser.parse_args()
if args.prot_name is None:
	args.prot_name = os.path.basename(args.protein).split('_')[0]
if args.lig_name is None:
	args.lig_name = os.path.basename(args.ligand).split('_')[0]

Atom_Keys = pd.read_csv(args.pdb_atom_keys_file, sep=",")
if args.desc_type == 'ecif':
	final = GetECIF(args.protein, args.ligand, distance_cutoff=6.0)
elif args.desc_type == 'elem':
	final = GetELEMENTS(args.protein, args.ligand, distance_cutoff=6.0)
elif args.desc_type == 'lig':
	final = list(GetRDKitDescriptors(args.ligand))
else:
	final = GetECIF(args.protein, args.ligand, distance_cutoff=6.0) + GetELEMENTS(args.protein, args.ligand, distance_cutoff=6.0) + list(GetRDKitDescriptors(args.ligand))

df_out = pd.DataFrame([args.prot_name, args.lig_name, final]).T
df_out.columns = ['pdb_id', 'lig_id', 'desc']
df_out.to_csv('ecif_%s.csv'%args.desc_type)
