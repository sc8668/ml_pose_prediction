...
def ecif_calculate(pdbname, ligname):
	...
	if ligname == '1_000x':		
		...
		cmd += 'python %s/ecif_calculate.py --pdb_atom_keys_file %s/PDB_Atom_Keys.csv -p ../../%s_prot/%s_p.pdb -l ../../%s_prot/%s_l.sdf --prot_name %s --lig_name %s -d elem'%(SCRIPT_dir, SCRIPT_dir, pdbname, pdbname, pdbname, pdbname, pdbname, ligname)
	else:	
		...
		cmd += 'python %s/ecif_calculate.py --pdb_atom_keys_file %s/PDB_Atom_Keys.csv -p ../../%s_prot/%s_p.pdb -l ./%s.sdf --prot_name %s --lig_name %s -d elem'%(SCRIPT_dir, SCRIPT_dir, pdbname, pdbname, ligname, pdbname, ligname)	
